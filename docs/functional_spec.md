...
|---------|--------|----------|-----------|
| USR-001 | CSV/XLSX 파일 업로드 | 사용자가 분석할 CSV 또는 Excel 파일을 업로드한다. | `frontend/src/pages/UserAnalysis.tsx` → `POST /api/analyze` |
| USR-002 | 분석 실행 | 업로드된 파일을 기준으로 분석을 시작한다. | `frontend/src/pages/UserAnalysis.tsx` → `POST /api/analyze` |
| USR-003 | 컬럼 선택 | 컴프, 전압, RT 등 분석에 사용할 주요 컬럼을 선택한다. | `frontend/src/pages/UserAnalysis.tsx` |
| USR-004 | 분석 결과 표시 | PASS, 관리필요, FAIL, 데이터품질이상 결과를 화면에 표시한다. | `frontend/src/pages/UserAnalysis.tsx` |
| USR-005 | 그래프 표시 | 주요 데이터 컬럼을 시계열 그래프(recharts)로 표시한다. | `frontend/src/pages/UserAnalysis.tsx` (`series` 데이터) |
| USR-006 | 이상 구간 표시 | Trip 발생/baseline 이탈 구간을 그래프(ReferenceArea)와 표에 표시한다. | `frontend/src/pages/UserAnalysis.tsx` (`trip.ranges`) |

## 2. 분석 기능 (ANA) — 백엔드

| 기능 ID | 기능명 | 기능설명 | 코드 구현 |
|---------|--------|----------|-----------|
| ANA-001 | 파일 파싱 | CSV/XLSX 파일을 읽고 데이터프레임 형태로 변환한다. | `backend/src/file_parser.py` |
...
| 기능 ID | 기능명 | 기능설명 | 코드 구현 |
|---------|--------|----------|-----------|
| DB-001 | 파일 정보 저장 | 업로드 파일명, 경로, 업로드 시간, 행 수, 분석 상태를 저장한다. | `backend/src/db_manager.py` |
| DB-002 | 분석 결과 저장 | 분석 판정, 이상 항목, Trip 정보, 리포트 내용을 저장한다. | `backend/src/db_manager.py` |
| DB-003 | Trip Code DB 저장 | Trip Code별 설명, 원인, 조치 가이드를 저장한다. | `backend/src/db_manager.py` |
| DB-004 | 정상 기준 DB 저장 | baseline 기준값과 feature별 정상 범위를 저장한다. | `backend/src/db_manager.py` |
| DB-005 | Prompt 저장 | 리포트 생성용 프롬프트 버전을 저장한다. | `backend/src/db_manager.py` |
...
| 기능 ID | 기능명 | 기능설명 | 코드 구현 |
|---------|--------|----------|-----------|
| ADM-001 | 사용자/엔지니어 화면 분리 | 일반 사용자는 분석 화면만, 엔지니어는 관리 화면까지 접근하도록 구분한다. | `frontend/src/components/Layout.tsx` + `frontend/src/context.tsx` |
| ADM-002 | 분석 이력 조회 | 과거 업로드 파일과 분석 결과를 다시 조회한다. | `frontend/src/pages/History.tsx` → `GET /api/history` |
