...
def _is_sequential_index_row(values: list) -> bool:
    ...
    일부 CSV/엑셀 원본은 1행에 컬럼 인덱스 번호가, 실제 헤더는 2행에 있다.
    """
    try:
...
def parse_csv(content: bytes) -> pd.DataFrame:
    ...
    for encoding in ("utf-8-sig", "cp949"):
        try:
            ...
            first_row = pd.read_csv(buffer, header=None, nrows=1, encoding=encoding)
            header_row = 1 if _is_sequential_index_row(first_row.iloc[0].tolist()) else 0
            buffer.seek(0)
            return pd.read_csv(buffer, header=header_row, encoding=encoding)
